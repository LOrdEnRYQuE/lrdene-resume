...
import { getStrongBlogSlugs, shouldIndexServicePage } from "@/lib/seo/indexing";
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    const [postsResult, projectsResult, servicesResult] = await Promise.all([
  ...
  const strongBlogSlugs = getStrongBlogSlugs();

  const postEntries = posts
    .filter((post) => strongBlogSlugs.has(post.slug))
    .map((post) => ({
    path: `/blog/${post.slug}`,
  ...
  const serviceEntries = services
    .filter((service) => shouldIndexServicePage({ slug: service.slug, hasLocationVariant: false }))
    .map((service) => ({
  ...
  const staticRoutes = [
    ...
    { path: "/faq", lastModified: staticLastModified, changeFrequency: "monthly" as const, priority: 0.8 },
